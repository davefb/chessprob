...
class Move:
    ...
    def shortAlgebraic(self):
        ...
        symbol = self.piece.symbol[WHITE];
        if symbol == 'P':
            symbol = '';
            
        return symbol  + `self.end` + suffix;
...
class Game:
    ...
    def __init__(self, white="white", black="black"):
        self.state = 'playing'
    ...
    def __str__(self):
        return "%s%d" % (self.board , self.turn)

    def __hash__(self):
        return object.__hash__(self.__str__());
    ...
    def addboard(self):
        self.boardstate = self.boardstate[:self.boardstate_index];
        self.saveboard();
        self.boardstate_index = self.boardstate_index + 1;

    def takeback(self):
        if self.boardstate_index == 0:
            return;

        self.boardstate_index = self.boardstate_index - 1;
        state = self.boardstate[self.boardstate_index-1]
        self.board.set(state[0]); self.turn = state[1];


    def saveboard(self):
        self.boardstate.append( ( "%s" % self.board , self.turn) );
    ...
    def move(self, command,  advance = True):
...
FEATURE_STRING = "feature analyze=1 sigint=0 usermove=1 colors=0"
class XBoardGame(Game):
    def __init__(self, white="white", black="black"):
        Game.__init__(self,white,black)
    ...
    def init(self):
        self.say(FEATURE_STRING);

    # this should be called programmatically
    # to send a move to XBoard
    def move(self, command,  advance = True):
        Game.move(self, command, advance);
        self.say(move);

    # ... but this should be called internally
    def _recievemove(self,themove):
        Game.move(self, command, advance);

    
    def xboard_xboard(self,*args):
        pass

    def xboard_force(self,*args):
        self.forcemode = True;
        # send engine notification...

    def xboard_go(self,*args):
        self.forcemode = False;
        # send engine notification...

    def xboard_ping(self, *args):
        self.say("pong %s" % args[0]);

    def xboard_draw(self, *args):
        pass

    def xboard_protover(self,*args):
        self.init()

    def xboard_variant(self,*args):
        pass;

    def xboard_usermove(self,*args):
        [move] = args;
        try:
            self._recievemove(self,move);
        except:
            self.say("Illegal move");
        
        
    def xboard_new(self, *args):
        self.clear()

    def xboard_post(self,*args):
        self.postflag = True;

    def xboard_nopost(self, *args):
        self.postflag = False;

    def xboard_quit(self, *args):
        self.stop = True;

    def xboard_undo(self, *args):
        self.internalState.takeback();

    def processCommand(self,cmd):
        t = cmd.strip();
        tsplit = t.split(" ");
        [cmd,args] = [ tsplit[0], tsplit[1:] ];

        toeval = "self.xboard_%s(%s)" % ( cmd , reduce( lambda x,y: "%s,%s" % (x,y) , map ( lambda x: "\"%s\"" % x)));

        try:
            eval(toeval,locals());
        except NameError:
            print "%s not defined" % toeval
                                     



    #spit a command to XBoard
    def say(self,tosay):
        if type(tosay) == str:
            tosay = [tosay]
...
def takeback_test():
    ...
    g.takeback();
    g.takeback();
    assert ('%s' %g)  == 'rnbqkb.r pppp.ppp .....n.. ....p... ....P... .....N.. PPPP.PPP RNBQKB.R0'
